...
async function loadDynamicArticles() {
    const insightsCarousel = document.querySelector('.insights-carousel');

    if (!insightsCarousel) return;

    try {
        // Determine the correct API URL based on environment
        const apiUrl = window.location.hostname === 'localhost'
            ? 'http://localhost:3000/api/articles?status=published&limit=3'
            : '/api/articles?status=published&limit=3';

        // Fetch published articles from the CMS API
        ...
        articles.forEach(article => {
            ...
            card.innerHTML = `
                ...
                <a href="/articles/${article.slug}" class="insight-link">Read More →</a>
            `;

            insightsCarousel.appendChild(card);
        });
